...
public class AwayMatchesFragment extends Fragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		List<ChampionshipStatis> cache = new HistoryDAO().getAwayMatches(getActivity(), StringUtils.stripAccents(favorite.getName().toLowerCase().trim())
				.replaceAll("-", ""));
	...
	private void buildView(List<ChampionshipStatis> cache) {
		...
		((TextView) getView().findViewById(R.id.titledsfsd)).setText("Número de jogos fora de casa(1971-2013)");
		listView.setAdapter(new ChampStatisAdapter(getActivity().getBaseContext(), R.layout.fragment_round_matches_list, cache, "away"));
	}
