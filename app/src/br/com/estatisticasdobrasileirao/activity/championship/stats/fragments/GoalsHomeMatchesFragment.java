...
public class GoalsHomeMatchesFragment extends Fragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		List<ChampionshipStatis> cache = new HistoryDAO().getHomeProGoals(getActivity(), StringUtils.stripAccents(favorite.getName().toLowerCase().trim())
				.replaceAll("-", ""));
	...
	private void buildView(List<ChampionshipStatis> cache) {
		...
		((TextView) getView().findViewById(R.id.titledsfsd)).setText("Gols feitos em casa(1971-2013)");
		
		listView.setAdapter(new ChampStatisAdapter(getActivity().getBaseContext(), R.layout.fragment_round_matches_list, cache,"goalsHome"));
	}
}
